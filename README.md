...
По умолчанию создается конфиг на одного пользователя с протоколом - VLESS, безопасность(маскрировка) - Reality (cloudflare.com:443), порт - 443. Есть смысл сначала просканировать сеть и вставить в скрипт нужные SNI. Чтобы потом не пересобирать vless ссылки вручную.

# Минимальные требования (чистый xray-core + amneziaWG cli)
Проверено на debian 13.
- ОС: ubuntu/debian
- Ресурсы: 512 МБ ОЗУ, 3-5 ГБ диска

Замеры делал через top. Скорее всего, в ubuntu можно удалить лишние пакеты.
